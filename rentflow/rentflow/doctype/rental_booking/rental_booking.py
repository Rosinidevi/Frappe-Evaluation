...
class RentalBooking(Document):
        ...
        for item in self.items:
            ...
            conflicts = frappe.db.sql(
                ...
                where rb.docstatus = 1
                  and rb.status NOT IN ('Cancelled', 'Returned') and rb.name != %(booking_name)s and bi.equipment_unit = %(equipment_unit)s
                  and rb.start_date <= %(end_date)s and rb.end_date >= %(start_date)s
                """,
                {"booking_name": self.name,"equipment_unit": item.equipment_unit,"start_date": self.start_date,"end_date": self.end_date,},
                as_dict=True,
            )
            if conflicts:
                frappe.throw(f"Equipment Unit {item.equipment_unit} " f"is already booked in {conflicts[0].name}")
    ...
    def on_submit(self):
        ...
        frappe.enqueue(method=send_booking_confirmation_email,booking_name=self.name,queue="short")

    def create_rental_invoice(self):
        existing_invoice = frappe.db.exists("Rental Invoice",{"rental_booking": self.name})
        if existing_invoice:
            return
    ...
    def on_trash(self):
        if self.status not in ("Cancelled", "Draft"):
            frappe.throw(
                "Only Draft or Cancelled Rental Bookings can be deleted."
            )
